...
###  A logster parser that can reports counts of HTTP methods and response
###  codes found in an Apache access log.
###
###  For example:
###  sudo ./logster --dry-run --output=ganglia HttpLogster /var/log/httpd/access_log
###
###
...
class HttpLogster(LogsterParser):
    ...
    def __init__(self, option_string=None):
        ...
        self.reg = re.compile('.*"(?P<method>{methods})?.* HTTP/1.\d\" (?P<http_status_code>\d{{3}}) .*'.format(methods="|".join(methods)))
